...
#If we simply LOOK at the histogram we can tell the (y) is not normally distributed (histogram of response variable):
...
plot(glmm.mod3)
  # This still isn't ideal, but it is MUCH better than before. Still are not normal but a tad better.
summary(glmm.mod3)
  # Our overall results are about the same significance. - P value changed for human a little bit.

#I need these packages to do this: `performance`, ‘bayestestR’, 'datawizard’ down below 
r.squaredGLMM(glmm.mod3)
  ...
  # A linear model assumes normality, so we cannot do anything! We need to move onto something that is now non linear
...
# The only difference in syntax from our glmmPQL will be how random variables are specified. 
  ...
  # Inside here was the same thing we started with on every function, random effect has different syntax now
    #We also assume that the data is non-linear now, which it is
...
# Our residuals look MUCH better. I've forced the ylim to zoom in and compare with the next model. Super random plot! YAY!
