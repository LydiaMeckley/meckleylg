...
brookie_events <- merge(all_brookies, events_meta)
              ...
              ###getting close to data we can actually play with###
...
#flashiness index# index value for year fish were sampled so we can connect it to

#https://cran.r-project.org/bin/windows/Rtools/.
#rtools 45 installer in the middle of the page - download (4.5 from the website )
  #or just run again a few mins later because it was just overloaded
...
colnames(R_B_HUC)[3] <- "RBI" #rename column
  ...
  ###now we dont have a away to associate them... jk look below###
...
#finaly going back to brookie data - turning them all into spatial data frames (spatial stuff is geometry)
...
#NOW WE CAN MAKE A COMPARISON - NUMBER OF BROOK TROUT RELATE TO FASHINESS?
...
summary(mod)
  #not too great, just a tiny pattern, thats okay

mod2 <- lm(BigCount~RBI, data = fish_flow)
summary(mod2)
  #adults are much more strongly affected (compared?) by flashiness

mod3 <- lm(SmallCount~RBI, data = fish_flow)
summary(mod3)
  #the little guys are even better!

mod4 <- lm(YOYRatio~RBI, data = fish_flow)
summary(mod4) #winner winner chicken dinner
  #independent the size of the stream, how much of the community will appear due to flashiness
  #R squared is what we should be looking at .2 is pretty wonderful



library(itsadug)
plot(mod4$residuals)#residuals from the YOYRatio lm() above
  #there is some spread, but overall looks pretty good
  #we can also look at if water quality has an influence

library(mgcv)
gam.mod <- gam(YOYRatio~RBI, data = fish_flow, na.action = na.omit, method = "REML")#RBI only
summary(gam.mod)
AIC(gam.mod) #83.94
  #high ratio (at the top) more babies, less babies
  #more flashy the system, the more babies (bottom axis is babies)
...
plot_smooth(gam.mod, view="Alk", rm.ranef=FALSE, ylab = "", xlab = "Specific Conductivity")
    #wider confidence interval (marginal)

#not good anymore
gam.mod <- gam(YOYRatio~RBI+Alk+SpecCond, data = fish_flow, na.action = na.omit, method = "REML")#RBI, alk, and specific conductivity
summary(gam.mod)
AIC(gam.mod) #81.44
  #this is how many salts are in the water - this increased alkalinity and improved r squared and AIC
