...
data(dune.env)
  ...
  #each column is a different species, and every row is a sample#
...
# Species "driving" the relationship will be the furthest from the origin.
  #the row names are the numbers
    #the closer the values are to each other, the more similar they are to one another. That goes with species too...
    #The species that are closer together are more likely to be found closer to each other IRL and closer to whatever that number is.
    #The ones farther away are least similar, which means they are least likely to be found by each other IRL
    #the farther they are from the origin in a 2D space????????????????????????????????????????????????????????????????????????????????????
...
#dune is a matric, and Use is using a single variable from the environment
  #the distance of these species compared with the use of land
mod1 <- rda(dune ~ Use, dune.env)
mod1
#The model call gives us constrained and unconstrained variance
# Constrained is the percent of the axes that is explained by "Use" - how much is all that variablity is constrained or explained by land use - 12% (so the rest of the percentage is NOT explained by it, which comes from the unconstrained section)
...
anova(mod1)
# This relationship is not significant. (the Pr f thing is P-Value)
...
# We can see what the cumulative effect of management and soil moisture is on the plant community: (moisture and management together)
...
# We have gone from 33% to 55% of the variance explained.
  #explaining over half of the variability - and it is significant

  #it's easy to test a model and just look at the significance of it, but we have 13 dimensional spaces, so to actually understand the pattern, we need to actually see things
#Let's plot to see what this looks like visually:
plot(mod3)
  #what's on here is how that moisture and management techniques relates to how the sites are curved (16 had a different moisture content than all the others)
    #the ones with an arrow are significant, and with no arrow they are not significant
    #we can see the differences in management techniquiees
    #it isnt that they are superimposed on each other, just that they use very similar management and moisture level techniques
...
# Next we will add a layer where the points are displayed with a label that represents their management - only display samle sites:
text(mod3, display="sites", labels = as.character(dune.env$Management))
  #you can start to see patterns in what management techniques are similar or different to others (closer vs farther)
...
# We will draw 95% confidence intervals around the "centroid" (average score) for each management group. - plot the average center point (mean x value and mean y value) - then take variance of that and get standard error
...
pl <- ordiellipse(mod3, dune.env$Management, kind="se", conf=0.95, lwd=2, draw = "polygon", 
    ...
    #the things in the blue circles are significant, and are different from the other groups - axis 1 (the x axis) is the highest most explained value - axis 2 is where they are completely different
...
# but very different on Axis 2 patterns.
  #gives the area of the confidence interval - NM is more consistent (smallest area) - SF is more variable (largest area)
...
head(Site_Scores)
  ...
  #use these values to spin and manipulate our plot
...
plot(Site_Scores[,1:3], type="n")
text(Site_Scores[,1:3], labels = as.character(dune.env$Use))
pl <- ordiellipse(Site_Scores[,1:3], dune.env$Use, kind="se", conf=0.95, lwd=2, draw = "polygon", 
                  col="skyblue", border = "blue")

# Rarefaction #### - also called rank abundance - the more individuals you sample in the community, the more species you are able to find - so this is the math behind it
  #how many species you get for how many individuals you sampled - eventually you will run out of species (look at curve I drew)
...
#now we can show what sample size we want
rarefaction(BCI, subsample=50, plot=TRUE, color=TRUE, error=FALSE,  legend=TRUE, symbol)
# This plot is very hard to read with this many samples. - 50 diff samples plotted ontop of each other, not very helpful
...
# But because species need to be columns we then transpose the data back, and make sure it is read as.data.frame()
  #we want the sums of all the columns to create our samples - combine all our samples
...
# While the curve really is steeper the difference is not quite as dramatic as it appears, consider the much larger range of the x-axis.
  #each dot is a new sample added
...
# How to apply rarefaction to real data all depends on the question you have relative to diversity and/or sampling effort.
  ...
  #compare locally, lots of small samples
